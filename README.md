...
- `mistral-medium-latest` handles SQL generation inside the tool only
...
SQL queries are generated by `mistral-medium-latest` using a few-shot prompt injected with:
...
|---|---|---|
| Model split | `sql_llm` = `mistral-medium-latest` (separate bucket) | Doubles effective rate limit for double-tool turns |
